...
function notify_devotion_author($postID, $data) {
  $post = get_post($postID);
  $devotion = get_post($postID);
  ...
  $fields = get_fields($postID);
    ...
    if ($devotion->devotional_author) {
      $devotion_author_email = get_field( 'email', $devotion->devotional_author->ID );
    }

  $to = $devotion_author_email;
  $subject = 'New Feedback';
  $headers = array('Content-Type: text/html; charset=UTF-8', 'From: Tucson Baptist Church <info@tucsonbaptist.com>');
